...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private int mHeaderTipsHeight;
	private final Handler mHandler = new Handler();

	private OnRefreshListener mOnRefreshListener;
	private OnRefreshListener2 mOnRefreshListener2;
	private OnShowLoadingLayoutListener onShowLoadingLayoutListener;

	private SmoothScrollRunnable mCurrentSmoothScrollRunnable;
	
	private OnPullYListener mOnPullYListener;
	
    //本次下拉刷新是否已做对OnShowLoadingLayoutListener的处理
    private boolean mIsDealtRefresh = false;
	
	public interface OnPullYListener
	{
		public void onPullY(int offset);
	}
	...
	public void setOnPullYListener(OnPullYListener listener)
	{
		mOnPullYListener = listener;
	}
			...
			case MotionEvent.ACTION_UP: {
				...
				if (mIsBeingDragged) {
					...
					if (mState == RELEASE_TO_REFRESH) {
					    ...
					    //如果下拉但没有更新
					    if(onShowLoadingLayoutListener != null)
		                {
		                    onShowLoadingLayoutListener.onHideHeadLayout();
		                }
					}
...
//			}
	...
	public final void setOnShowLoadingLayoutListener(OnShowLoadingLayoutListener listener) {
	    onShowLoadingLayoutListener = listener;
    }
	...
	protected void onPullYListener(int y)
	{
		if(mOnPullYListener != null && mCurrentMode == Mode.PULL_DOWN_TO_REFRESH)
		{
			mOnPullYListener.onPullY(y);
		}
	}
	
	protected void setRefreshingHeaderInternal()
	{
		if (!isRefreshing()) {
			mState = REFRESHING;
    ...
    private boolean pullEvent() {
		...
		switch (mCurrentMode) {
			...
			case PULL_DOWN_TO_REFRESH:
			default:
			    if(!mIsDealtRefresh && onShowLoadingLayoutListener != null)
			    {
			        mIsDealtRefresh = true;
			        onShowLoadingLayoutListener.onShowHeadLayout();
			    }
		...
		if (newHeight != 0) {
			...
			float scale = Math.abs(newHeight) / (float) mHeaderTipsHeight;
			switch (mCurrentMode) {
				case PULL_UP_TO_REFRESH:
			...
			if (mState == PULL_TO_REFRESH && mHeaderTipsHeight < Math.abs(newHeight)) {
				mState = RELEASE_TO_REFRESH;
				onReleaseToRefresh();
				return true;

			} else if (mState == RELEASE_TO_REFRESH && mHeaderTipsHeight >= Math.abs(newHeight)) {
				mState = PULL_TO_REFRESH;
				onPullToRefresh();
				return true;
			}
	...
	private void refreshLoadingViewsHeight() {
		if (mMode.canPullDown()) {
			...
			mHeaderTipsHeight = mHeaderLayout.getLoadingTipsHeight();
	...
	/**
	 * 展示、隐藏HeaderLayout和FooterLayout时回调
	 * @author wangwei27
	 */
    public static interface OnShowLoadingLayoutListener {

        public void onShowHeadLayout();
        
        public void onHideHeadLayout();
